...
public class LumosNNThroughputBenchmark implements Tool {
  ...
  // static NamenodeProtocols nameNode;
  public static short REPLICATION = 2;
  public static ClientProtocol nameNode;

  public LumosNNThroughputBenchmark(Configuration conf) throws IOException {
  ...
  abstract class OperationStatsBase {
    ...
    OperationStatsBase() {
      ...
      replication = LumosNNThroughputBenchmark.REPLICATION;
      numOpsRequired = 10;
      numThreads = 3;
  ...
  class MkdirsStats extends OperationStatsBase {
    ...
    long executeOp(int daemonId, int inputIdx, String clientName)
        throws IOException {
      long start = Time.now();
      nameNode.mkdirs(dirPaths[daemonId][inputIdx],
  ...
  class AppendFileStats extends OpenFileStats {
    ...
    static final String OP_APPEND_USAGE = "-op " + OP_APPEND_NAME + OP_USAGE_ARGS + " [" + APPEND_NEW_BLK + ']';
  ...
  private static class TinyDatanode implements Comparable<String> {
    ...
    void register() throws IOException {
      ...
      //};
      //nameNode.blockReport(dnRegistration, 
    ...
    void sendHeartbeat() throws IOException {
      ...
      // DatanodeCommand[] cmds = nameNode.sendHeartbeat(dnRegistration, rep,
  ...
  class BlockReportStats extends OperationStatsBase {
    ...
    void generateInputs(int[] ignore) throws IOException {
      ...
      for(int idx=0; idx < nrFiles; idx++) {
        ...
        nameNode.complete(fileName, clientName, lastBlock, INodeId.GRANDFATHER_INODE_ID);
      }
  ...
  public static void runBenchmark(Configuration conf, List<String> args)
    ...
    try {
      bench = new LumosNNThroughputBenchmark(conf);
  ...
  public int run(String[] aArgs) throws Exception {
    ...
    //nameNode = nameNode.getRpcServer();

    try {
      nameNode = NameNodeProxies.createProxy(new HdfsConfiguration(), 
          new URI("hdfs://localhost:9000"), ClientProtocol.class).getProxy();
